...
/*
	if (t==NULL){
   	...
   	if (color(left(t))==RED&&color(left(t))==RED){	//if the current node is four-node, then split	 
   		color(t)=RED;
   		color(left(t))=BLACK;
   		color(right(t))=BLACK;
	}
	if (it<data(t)){
		...
		if (inRight==0&&color(t)==RED&&color(right(t))==RED){
			t=rotateLeft(t);
		}
		if (color(left(t))==RED&&color(left(left(t)))==RED){
			t=rotateRight(t);
			color(t)=BLACK;
			color(right(t))=RED;
		}
		...
		// re-arrange links/colors after insert
		if (inRight==1&&color(t)==RED&&color(left(t))==RED){
			t=rotateRight(t);
		}
		if (color(right(t))==RED&&color(right(right(t)))==RED){
			t=rotateLeft(t);
			color(t)=BLACK;
			color(left(t))=RED;
		}
